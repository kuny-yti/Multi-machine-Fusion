...
int64_t audio_output::status(bool *need_data)
{
    ...
    else
    {
        ...
        if (timestamp != _last_timestamp)
        {
            ...
            _ext_timer_at_last_timestamp = timer_::get_microseconds(timer_::monotonic);
            _last_reported_timestamp = std::max(_last_reported_timestamp, timestamp);
            return _last_reported_timestamp;
        }
        else
        {
            _last_reported_timestamp = _last_timestamp + (timer_::get_microseconds(timer_::monotonic) - _ext_timer_at_last_timestamp);
            return _last_reported_timestamp;
        }
    }
}
