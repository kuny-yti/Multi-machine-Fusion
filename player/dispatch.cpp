...
dispatch::dispatch(msg::level_t log_level,
                   bool benchmark, int swap_interval) throw () :
    _audio_output(NULL), _video_output(NULL),
    _media_input(NULL), _player(NULL),
...
void dispatch::video_output(class video_output* v)
{
    if (!global_dispatch) return;
    global_dispatch->_video_output = v;
}
bool dispatch::playing()
{
    if (!global_dispatch) return false;
...
{
    bool early_quit = early_quit_is_allowed();
...
void dispatch::receive_cmd(const command& cmd)
{
    ...
    case command::open:
        ...
        //********自行加入的，以后删除。设置为只有2D模式***********//
        _parameters.set_stereo_layout(parameters::layout_mono);
        _media_input->set_stereo_layout(_parameters.stereo_layout(),
                _parameters.stereo_layout_swap());
        //*******************************************************//
