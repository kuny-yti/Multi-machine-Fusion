...
static void compute_viewport_and_tex_coords(int vp[4], float tc[4][2],
        ...
        float crop_ar, float zoom, bool need_even_width,
        bool need_even_height)
{
...
void video_output::color_init(int index, const parameters& params, const video_frame &frame)
{
    ...
    file.close();
    std::string color_fs_src(color.toLatin1().data());//(VIDEO_OUTPUT_COLOR_FS_GLSL_STR);
...
void video_output::render_init()
{
    ...
    QString render;
    QFile file(":/shader/video_output_render.fs.glsl");
    file.open(QIODevice::ReadOnly);
    render = file.readAll();
    file.close();

    std::string render_fs_src(render.toLatin1().data());//(VIDEO_OUTPUT_RENDER_FS_GLSL_STR);
        ...
        parameters::stereo_mode_t stereo_mode)
{
    ...
    bool context_needs_stereo = (_render_params.stereo_mode()
                                      ...
                                      const subtitle_box &subtitle)
{
    ...
    if (!_color_prg[index] ||
            !color_is_compatible(index, dispatch::parameters(), frame))
    {
        //color_deinit(index);
