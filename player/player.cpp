...
int64_t player::step(bool *more_steps, int64_t *seek_to, bool *prep_frame, bool *drop_frame, bool *display_frame)
{
    ...
    *display_frame = false;
    ...
    else if (!_running)
    {
        ...
        {
            _master_time_start = timer_::get_microseconds(timer_::monotonic);
            _master_time_pos = _video_pos;
            _current_pos = _video_pos;
        }
        _start_pos = _current_pos;
        _fps_mark_time = timer_::get_microseconds(timer_::monotonic);
        _frames_shown = 0;
        _running = true;
    ...
    else if (_pause_request)
    {
        if (!_in_pause)
        {
            ...
            {
                _pause_start = timer_::get_microseconds(timer_::monotonic);
            }
            _in_pause = true;
    ...
    else
    {
        if (_in_pause)
        {
            ...
            {
                _master_time_start += timer_::get_microseconds(timer_::monotonic) - _pause_start;
            }
            _in_pause = false;
                ...
                && global_dispatch->get_media_input()->selected_audio_stream() >= 0)
        {
            ...
            _master_time_current = global_dispatch->get_audio_output()
                    ->status(&need_audio_data)
                    - _master_time_start + _master_time_pos;
            // Output requested audio data
            if (need_audio_data)
            {
                audio_blob blob = global_dispatch->get_media_input()
                        ->finish_audio_blob_read();
                if (!blob.is_valid())
                {
                ...
                _master_time_pos = _audio_pos;
                global_dispatch->get_audio_output()->data(blob);
                global_dispatch->get_media_input()->
                        start_audio_blob_read(global_dispatch->get_audio_output()->required_update_data_size());
                _current_pos = _audio_pos;
                global_dispatch->set_position(normalize_pos(_current_pos));
            }
        }
        ...
        {
            // Use our own timer
            _master_time_current = timer_::get_microseconds(timer_::monotonic)
                    - _master_time_start + _master_time_pos;
        }
            ...
            if (!_previous_frame_dropped)
            {
                *display_frame = true;
                ...
                if (dispatch::parameters().benchmark())
                {
                    ...
                    {
                        int64_t now = timer_::get_microseconds(timer_::monotonic);
