...
BOOL APIENTRY DllMain(HINSTANCE hModule,DWORD ul_reason_for_call,LPVOID lpReserved)
{
    ...
    switch(ul_reason_for_call)
    {
        ...
        case DLL_THREAD_DETACH:
            ...
            {
                void *ptr = pthread_getspecific_(TlsDestructor.array[i].key);
...
static void alc_init(void)
{
    ...
    pthread_key_create_(&LocalContext, ReleaseThreadCtx);
...
static void alc_deinit_safe(void)
{
    ...
    pthread_key_delete_(LocalContext);
...
static void ReleaseContext(ALCcontext *context, ALCdevice *device)
{
    ...
    if(pthread_getspecific_(LocalContext) == context)
    {
        WARN("%p released while current on thread\n", context);
        pthread_setspecific_(LocalContext, NULL);
...
ALCcontext *GetContextRef(void)
{
    ...
    context = pthread_getspecific_(LocalContext);
    if(context)
...
ALC_API ALCboolean ALC_APIENTRY alcMakeContextCurrent(ALCcontext *context)
{
    ...
    if((context=pthread_getspecific_(LocalContext)) != NULL)
    {
        pthread_setspecific_(LocalContext, NULL);
        ALCcontext_DecRef(context);
    }
...
ALC_API ALCboolean ALC_APIENTRY alcSetThreadContext(ALCcontext *context)
{
    ...
    /* context's reference count is already incremented */
    old = pthread_getspecific_(LocalContext);
    pthread_setspecific_(LocalContext, context);
