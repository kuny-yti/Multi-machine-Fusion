...
#ifdef _WIN32
void pthread_once_(pthread_once_t *once, void (*callback)(void))
{
    LONG ret;
...
int pthread_key_create_(pthread_key_t *key, void (*callback)(void*))
{
    *key = TlsAlloc();
    if(callback)
...
int pthread_setspecific_(pthread_key_t key, void *val)
{
    TlsSetValue(key, val);
    return 0;
}
