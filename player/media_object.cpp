...
void read_thread::run()
{
    while (!_eof)
    {
        const size_t video_stream_low_threshold = (_is_device ? 1 : 5);
        const size_t audio_stream_low_threshold = (_is_device ? 1 : 10);
        const size_t subtitle_stream_low_threshold = (_is_device ? 1 : 1);
        bool need_another_packet = false;
        for (size_t i = 0;
             !need_another_packet && i < _ffmpeg->video_streams.size();
             i++)
        {
            if (_ffmpeg->format_ctx->streams[_ffmpeg->video_streams[i]]
                    ->discard == AVDISCARD_DEFAULT)
            {
                _ffmpeg->video_packet_queue_mutexes[i].lock();
                need_another_packet = _ffmpeg->video_packet_queues[i].size()
                        < video_stream_low_threshold;
                _ffmpeg->video_packet_queue_mutexes[i].unlock();
            }
        }
        for (size_t i = 0;
             !need_another_packet && i < _ffmpeg->audio_streams.size();
             i++)
        {
            if (_ffmpeg->format_ctx->streams[_ffmpeg->audio_streams[i]]
                    ->discard == AVDISCARD_DEFAULT)
            {
                _ffmpeg->audio_packet_queue_mutexes[i].lock();
                    need_another_packet = _ffmpeg->audio_packet_queues[i].size()
                            < audio_stream_low_threshold;
                    _ffmpeg->audio_packet_queue_mutexes[i].unlock();
            }
        }
        for (size_t i = 0;
             !need_another_packet && i < _ffmpeg->subtitle_streams.size();
             i++)
        {
            if (_ffmpeg->format_ctx->streams[_ffmpeg->subtitle_streams[i]]
                    ->discard == AVDISCARD_DEFAULT)
            {
                _ffmpeg->subtitle_packet_queue_mutexes[i].lock();
                need_another_packet = _ffmpeg->subtitle_packet_queues[i].size()
                        < subtitle_stream_low_threshold;
        ...
        if (e < 0)
        {
            ...
            {
                throw exc(str::asprintf(_("%s: %s"), _url.c_str(),
                                        my_av_strerror(e).c_str()));
            }
        }
        // Put the packet in the right queue.
        bool packet_queued = false;
        for (size_t i = 0;
             i < _ffmpeg->video_streams.size() && !packet_queued;
             i++)
        {
            if (packet.stream_index == _ffmpeg->video_streams[i])
            {
                ...
                {
                    throw exc(str::asprintf(_("%s: Cannot duplicate packet."),
                                            _url.c_str()));
                }
                _ffmpeg->video_packet_queue_mutexes[i].lock();
        ...
        for (size_t i = 0;
             i < _ffmpeg->audio_streams.size() && !packet_queued;
             i++)
        {
            if (packet.stream_index == _ffmpeg->audio_streams[i])
            {
                ...
                if (_ffmpeg->audio_packet_queues[i].empty()
                        && _ffmpeg->audio_last_timestamps[i] ==
                ...
                else
                {
                    ...
                    {
                        _ffmpeg->audio_packet_queue_mutexes[i].unlock();
                        throw exc(str::asprintf(
                                      _("%s: Cannot duplicate packet."),
                                      _url.c_str()));
                    }
        ...
        for (size_t i = 0;
             i < _ffmpeg->subtitle_streams.size() && !packet_queued;
             i++)
        {
            if (packet.stream_index == _ffmpeg->subtitle_streams[i])
            {
                ...
                if (_ffmpeg->subtitle_packet_queues[i].empty()
                        && _ffmpeg->subtitle_last_timestamps[i] ==
                        std::numeric_limits<int64_t>::min()
                        && packet.dts == static_cast<int64_t>(AV_NOPTS_VALUE))
                {
                ...
                else
                {
                    ...
                    msg::dbg(_url + ": "
                            + str::from(_ffmpeg
                                        ->subtitle_packet_queues[i].size())
                            + " packets queued in subtitle stream " +
                             str::from(i) + ".");
                }
                _ffmpeg->subtitle_packet_queue_mutexes[i].unlock();
            }
...
read_frame:
        do
        {
            ...
            _ffmpeg->video_packets[_video_stream] =
                    _ffmpeg->video_packet_queues[_video_stream].front();
        ...
        {
            msg::dbg(_("%s video stream %d: Dropping %dx%d frame"),
                     _url.c_str(), _video_stream + 1,
                    _ffmpeg->video_frames[_video_stream]->width,
                     _ffmpeg->video_frames[_video_stream]->height);
            goto read_frame;
        }
        ...
        else
        {
            ...
            if (_raw_frames == 2 && raw_frame == 0)
            {
                av_picture_copy(reinterpret_cast<AVPicture *>
                        ...
                        static_cast<enum PixelFormat>
                                (_ffmpeg->video_codec_ctxs[_video_stream]
                                 ->pix_fmt),
        ...
        if (_ffmpeg->video_packets[_video_stream].dts !=
                static_cast<int64_t>(AV_NOPTS_VALUE))
        {
            _frame.presentation_time = handle_timestamp(
                        _ffmpeg->video_packets[_video_stream].dts * 1000000
                    * _ffmpeg->format_ctx->streams[
                        _ffmpeg->video_streams[_video_stream]]->time_base.num
                    / _ffmpeg->format_ctx->streams[_ffmpeg
                    ->video_streams[_video_stream]]->time_base.den);
        }
        else if (_ffmpeg->video_last_timestamps[_video_stream] !=
                 std::numeric_limits<int64_t>::min())
        {
            msg::dbg(_url + ": video stream " + str::from(_video_stream)
                    + ": no timestamp available, using a questionable guess");
            _frame.presentation_time =
                    _ffmpeg->video_last_timestamps[_video_stream];
        }
        else
        {
            msg::dbg(_url + ": video stream " + str::from(_video_stream)
