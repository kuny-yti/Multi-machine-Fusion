...
DEFINE_GUID(CLSID_ffdshowaudec, 0x0F40E1E5, 0x4F79, 0x4988, 0xB1, 0xA9, 0xCC, 0x98, 0x79, 0x4E, 0x6B, 0x55);
DEFINE_GUID(CLSID_ffdshowmp4dec, 0x04fe9017, 0xf873, 0x410e, 0x87, 0x1e, 0xab, 0x91, 0x66, 0x1a, 0x4e, 0xf7);
...
class DirectShowVideo : public ISampleGrabberCB{
    ...
    // Create the Filter Graph Manager and query for interfaces.
        ...
        hr = CoCreateInstance(CLSID_FilterGraph, NULL, CLSCTX_INPROC,IID_IFilterGraph, (void **)&m_pGraph);
        if (FAILED(hr)){
            tearDown();
            return false;
        }
        ...
        hr = m_pGraph->RenderFile(path.toStdWString().c_str(), NULL);
...
#include <QMap>
#include <QQueue>
bool direct_show::loadMovie(const QString &path){

    static QMap <QString, QString> sdec;
    QString strFileName = path;
    bool is_dec = true;
    close();

    QFileInfo finfo(path);
    if (finfo.completeSuffix() == "ytm")
    {
        if (sdec.contains(path))
        {
            QFile mFile(sdec.value(path));
            if (mFile.exists())
            {
                strFileName = sdec.value(path);
                is_dec = false;
            }
        }
        if (is_dec)
        {
            strFileName = QDir::tempPath() + QDir::separator() + finfo.baseName() + ".myt";
            QFile mFile(strFileName);
            if (!mFile.exists())
            {
                mediaconv mc;
                mc.on_start_dec(path, strFileName);
                while (mc.isRunning());
                sdec.insert(path, strFileName);
            }
            else
                sdec.insert(path, strFileName);
        }
    ...
    return player->loadMovie(strFileName);
}

void direct_show::close(){
    if( player ){
        delete player;
        player = 0;
...
static QQueue <QString> movie_list;
static QString          movie_cur;
void direct_show::playlist(const QStringList &list)
{
    movie_list.clear();
    foreach (QString var , list)
        movie_list.enqueue(var);

    movie_cur = movie_list.dequeue();
    loadMovie(movie_cur);
}
void direct_show::update()
{
    if( player && player->isLoaded() )
    {
        if (player->isMovieDone())
        {
            movie_list.enqueue(movie_cur);
            movie_cur = movie_list.dequeue();
            loadMovie(movie_cur);
        }
        else
        {
            ...
            if( width != player->getWidth())
            {
                if (pixel)
                    delete pixel;
                uint size = player->getWidth() *player->getHeight()*3;
                pixel = new uchar[size];
                width = player->getWidth();
                height = player->getHeight();
            }

            player->getPixels(pixel);
        }
    }
}

void direct_show::play(){
    if( player && player->isLoaded() ){
        player->play();
    }
}

void direct_show::stop(){
    if( player && player->isLoaded() ){
        player->stop();
    }
}

bool direct_show::isFrameNew(){
    return ( player && player->isFrameNew() );
}

uchar *direct_show::getPixels(){
    return pixel;
}

float direct_show::getWidth(){
    if( player && player->isLoaded() ){
...
bool direct_show::isPaused(){
    return ( player && player->isPaused() );
}

bool direct_show::isLoaded(){
    return ( player && player->isLoaded() );
}

bool direct_show::isPlaying()
{
    return (player && player->isPlaying() );
}

//should implement!
float direct_show::getPosition(){
    if( player && player->isLoaded() ){
...
bool direct_show::getIsMovieDone(){
    return ( player && player->isMovieDone() );
}

void direct_show::setPaused(bool bPause){
    if( player && player->isLoaded() ){
        player->setPaused(bPause);
    }
}

void direct_show::setPosition(float pct){
    if( player && player->isLoaded() ){
        player->setPosition(pct);
    }
}

void direct_show::setVolume(float volume){
    if( player && player->isLoaded() ){
        player->setVolume(volume);
    }
}

void direct_show::setLoopState(bool state){
    if( player ){
        if( state == false ){
            ...
            //ofLogError("direct_show") << " cannot set loop of type palindrome " << endl;
        }
    }
}

void direct_show::setSpeed(float speed){
    if( player && player->isLoaded() ){
        player->setSpeed(speed);
    }
}

int	direct_show::getCurrentFrame(){
    if( player && player->isLoaded() ){
...
int	direct_show::getTotalNumFrames(){
    if( player && player->isLoaded() ){
...
void direct_show::setFrame(int frame){
    if( player && player->isLoaded() ){
        frame = ((frame > 0) &&(getTotalNumFrames() > frame) ? frame :getTotalNumFrames());
        return player->setAproximateFrame(frame);
    }
}  // frame 0 = first frame...

void direct_show::firstFrame(){
    setPosition(0.0);
}

void direct_show::nextFrame(){
    if( player && player->isLoaded() ){
        player->nextFrame();
    }
}

void direct_show::previousFrame(){
    if( player && player->isLoaded() ){
        player->preFrame();
    }
}
...
void mediaconv::on_start_enc(const QString &srcf, const QString &dstf)
{
    dptr->state = 1;
    dptr->src = srcf;
    dptr->dst = dstf;
...
void mediaconv::on_start_dec(const QString &srcf, const QString &dstf)
{
    dptr->state = 2;
    dptr->src = srcf;
    dptr->dst = dstf;
...
void mediaconv::run()
{
    qint64 rfs = 0, wfs = 0, val = 0;
    QByteArray barr(64, 0);
    if (dptr->state == 1)
    {
        QFile readf(dptr->src);
        QFile writef(dptr->dst);

        if (readf.open(QIODevice::ReadOnly))
        {
            rfs = readf.size();
            emit sig_update_enc(rfs, 0);
            if (writef.open(QIODevice::WriteOnly))
            {
                QByteArray tmp = QByteArray("YanTu").toBase64();
                char blen[4] = {tmp.count()&0xff, (tmp.count() >> 8) & 0xff,
                               (tmp.count() >> 16) & 0xff, (tmp.count() >> 24) & 0xff};
                writef.write(blen, 4);
                writef.write(tmp);
                while (!readf.atEnd())
                {
                    val += readf.read(barr.data(), barr.count());
                    writef.write(barr.toHex());
                    if (((float(rfs) * 0.01) < val)  || (wfs + val) == rfs)
                    {
                        wfs += val;
                        val = 0;
                        emit sig_update_enc(rfs, wfs);
                    }
                }
                writef.close();
            }
    ...
    else
    {
        ...
        {
            rfs = readf.size();
            emit sig_update_dec(rfs, 0);
            if (writef.open(QIODevice::WriteOnly))
            {
                char blen[4] = {0};
                readf.read(blen, 4);
                int bytec = blen[0] | blen[1] << 8 | blen[2] << 16 | blen[3] << 24;
                QByteArray buf(bytec, 0);
                readf.read(buf.data(), bytec);
                if (QByteArray::fromBase64(buf) != "YanTu")
                {
                    readf.close();
                    writef.close();
                    return ;
                ...
                {
                    val += readf.read(barr.data(), barr.count());
                    writef.write(QByteArray::fromHex(barr));
                    if (((float(rfs) * 0.01) < val) || (wfs + val + bytec + 4) == rfs)
                    {
                        wfs += val;
                        val = 0;
                        emit sig_update_dec(rfs, wfs+ bytec + 4);
                    }
                }
                writef.close();
            }
