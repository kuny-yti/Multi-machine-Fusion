...
static QColor line_color[12][12] =
{
    {
        {QColor(10, 10, 10)},
        {QColor(200, 0, 0)},
...
public:
    ...
    void mouseMove(QMouseEvent *e, QWidget *draw)
    {
        if ((back_select_h != -1) && (back_select_v != -1))
        {
            ...
            //! 通知更新数据
            ((area_adjust*)draw)->callback(Command_DeviceCut);
        }
        back_point = e->pos ();
        draw->update ();
    }
        ...
        if (device_v && device_h)
        {
            ...
            for (int i = 0; i < device_v; i++)
            {
                for (int j = 0; j < device_h; j++)
                {
                    ...
                    pai.setPen(QPen(line_color[i][j],1,Qt::DashDotDotLine)); //设置画笔形式
                    pai.setBrush(QBrush(line_color[i][j],Qt::NoBrush)); //设置画刷形式
                    pai.drawRect (tem);
                }
            }
            ...
            for (int i = 0; i < channel_data.size (); i++)
            {
                for (int j = 0; j < channel_data[i].size (); j++)
                {
                    ...
                    pai.setPen(QPen(line_color[i][j], 1,Qt::SolidLine)); //设置画笔形式
                    pai.setBrush(QBrush(line_color[i][j], Qt::NoBrush)); //设置画刷形式
...
area_adjust::area_adjust(QWidget *parent) :
    QWidget(parent),
...
void area_adjust::callback(int cmd)
{
    wParameBegin();
...
bool area_adjust::event(QEvent *evt)
{
...
void area_adjust::command(const int &cmd)
{
    switch (cmd)
    {
